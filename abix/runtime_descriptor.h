...
//
// The canonical_{types,layouts} arrays are the one ABI truth — they are
// generated by AMC and define the authoritative TypeDesc / TypeLayout for
// each type.  RuntimeRegistry uses them directly (no reconstruction) so
// there is exactly one source of ABI fact, not a re-derivation.
struct ModuleDescriptor {
    const char *name;                             //  8
    const char *version;                          //  8
    const TypeDescriptor *types;                  //  8  (runtime projection)
    const model::TypeDesc *canonical_types;       //  8  (one ABI truth)
    const model::TypeLayout *canonical_layouts;   //  8  (one ABI truth)
...
inline bool valid(const ModuleDescriptor &module) noexcept {
    ...
    if ((module.type_count != 0 && (!module.types || !module.canonical_types || !module.canonical_layouts))
        || (module.function_count != 0 && !module.functions)
        || (module.symbol_count != 0 && !module.symbols))
        return false;
    for (uint32_t i = 0; i < module.type_count; ++i) {
        ...
        // Canonical TypeDesc identity must match the runtime projection.
        if (type.type_id != canonical_type.id) return false;
        // Canonical TypeLayout size/align/layout_hash must be consistent.
        if (type.size != canonical_layout.size
            || type.align != canonical_layout.align
            || type.layout_hash != canonical_layout.layout_hash)
            return false;
        if (type.type_id == model::Hash128{} || type.size == 0 || type.align == 0) return false;
