...
static uint32_t count_physical_cores() {
    ...
    for (uint32_t i = 0; i < max_cpus; ++i) {
        char path[128];
        std::snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/topology/core_id", i);
        std::ifstream f(path);
        uint32_t core_id = 0;
        if (f >> core_id) {
            // Also get package ID
            char pkg_path[128];
            std::snprintf(pkg_path, sizeof(pkg_path), "/sys/devices/system/cpu/cpu%u/topology/physical_package_id", i);
...
std::string CpuTopology::to_string() const {
    ...
    if (performance_cpus > 0 || efficiency_cpus > 0) {
        s += " (" + std::to_string(performance_cpus) + "P + " + std::to_string(efficiency_cpus) + "E)";
    }
