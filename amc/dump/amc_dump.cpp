...
void json_string(std::ostream &output, const std::string &value) { output << '"' << escape_json(value) << '"'; }
...
void write_json(const amc::AbiModule &module, std::ostream &output) {
    ...
    for (size_t i = 0; i < hashes.size(); ++i) {
        ...
        output
            << "{\"kind\": "
            << static_cast<uint32_t>(hashes[i].kind)
            << ", \"target_kind\": "
            << static_cast<uint32_t>(hashes[i].target_kind)
            << ", \"target_index\": "
            << hashes[i].target_index
            << ", \"flags\": "
            << hashes[i].flags
            << ", \"value\": ";
    ...
    indent(output, 1);
    output
        << "\"runtime_descriptor\": {\"type_id_bits\": 128, \"registry\": \"RuntimeRegistry\", "
        << "\"type_count\": "
        << module.types.size()
        << ", \"function_count\": "
    ...
    for (size_t i = 0; i < module.types.size(); ++i) {
        ...
        output << ", \"id\": ";
        json_string(output, hash_string(type.id));
        output << ", \"layout_hash\": ";
        json_string(output, hash_string(type.layout_hash));
        output
            << ", \"size\": "
            << type.size
            << ", \"align\": "
    ...
    for (size_t i = 0; i < module.fields.size(); ++i) {
        ...
        output << ", \"owner_type\": ";
        json_string(output, hash_string(field.owner_type));
        output << ", \"type_id\": ";
        json_string(output, hash_string(field.type_id));
        output
            << ", \"offset\": "
            << field.offset
            << ", \"flags\": "
    ...
    for (size_t i = 0; i < module.functions.size(); ++i) {
        ...
        output << ", \"owner_type\": ";
        json_string(output, hash_string(function.owner_type));
        output << ", \"signature\": ";
        json_string(output, hash_string(function.signature));
        output << ", \"return_type\": ";
        json_string(output, hash_string(function.return_type));
        output
            << ", \"calling_convention\": "
        ...
        for (size_t p = 0; p < function.parameters.size(); ++p) {
            ...
            output << ", \"type_id\": ";
            json_string(output, hash_string(parameter.type_id));
    ...
    for (size_t i = 0; i < module.compatibility.size(); ++i) {
        ...
        output << ", \"source_type\": ";
        json_string(output, hash_string(record.source_type));
        output << ", \"target_type\": ";
        json_string(output, hash_string(record.target_type));
    ...
    for (size_t i = 0; i < module.maps.size(); ++i) {
        ...
        output << ", \"source_type\": ";
        json_string(output, hash_string(map.source_type));
        output << ", \"target_type\": ";
        json_string(output, hash_string(map.target_type));
        output
            << ", \"operation_begin\": "
            << map.operation_begin
            << ", \"operation_count\": "
...
void write_text(const amc::AbiModule &module, std::ostream &output) {
    ...
    output
        ...
        << "\n";
    output
        << "hash: algorithm=0 canonical_version=1 abi="
        << hash_string(amc::abi_hash(module))
        << " records="
        << amc::hash_table(module).size()
        << "\n";
    output
        << "runtime_descriptor: type_id_bits=128 registry=RuntimeRegistry types="
        << module.types.size()
        << " functions="
        << module.functions.size()
        << " symbols="
    ...
    for (const auto &record : module.compatibility)
        output
            ...
            << " "
            << hash_string(record.source_type)
            << " -> "
            << hash_string(record.target_type)
            << " map="
            << record.map_index
            << " flags="
            << record.flags
    ...
    for (const auto &map : module.maps) {
        output
            ...
            << map.source_name
            << " ("
            << hash_string(map.source_type)
            << ") -> "
            << map.target_name
            << " ("
            << hash_string(map.target_type)
            << ") begin="
            << map.operation_begin
            << " operations="
            << map.operations.size()
        ...
        for (size_t i = 0; i < map.operations.size(); ++i) {
            ...
            output
                ...
                << i
                << ") "
                << map_opcode_name(operation.opcode)
                << " source_field="
                << operation.source_field
                << " target_field="
                << operation.target_field
                ...
                << operation.byte_count
                << " auxiliary="
                << hash_string(operation.auxiliary)
                << '\n';
        }
    }
}
...
void write_diff_text(
    const amc::AbiModule &source, const amc::AbiModule &target, const amc::AbiModule &report, std::ostream &output) {
    output
        << "diff:\n  source: "
        << source.package_name
        << " abi="
        << hash_string(amc::abi_hash(source))
        << "\n  target: "
        << target.package_name
        << " abi="
        << hash_string(amc::abi_hash(target))
        << '\n';
...
void write_diff_json(
    ...
    output << ", \"abi_hash\": ";
    json_string(output, hash_string(amc::abi_hash(source)));
    output << "},\n  \"target\": {\"package\": ";
    json_string(output, target.package_name);
    output << ", \"abi_hash\": ";
    json_string(output, hash_string(amc::abi_hash(target)));
    output << "},\n  \"report\": ";
...
int main(int argc, char **argv) {
    ...
    if (diff) {
        ...
        std::ofstream output(args[5], std::ios::binary);
        if (!output) return fail(amc::ErrorCategory::io, "amc-dump", "cannot open JSON output", args[5]);
        write_diff_json(source, target, report, output);
        if (!output) return fail(amc::ErrorCategory::io, "amc-dump", "failed to write JSON output", args[5]);
        return 0;
    }
    ...
    if (!json) {
        write_text(module, std::cout);
        return 0;
    }
    std::ofstream output(args[3], std::ios::binary);
    if (!output) return fail(amc::ErrorCategory::io, "amc-dump", "cannot open JSON output", args[3]);
    write_json(module, output);
    if (!output) return fail(amc::ErrorCategory::io, "amc-dump", "failed to write JSON output", args[3]);
    return 0;
}
