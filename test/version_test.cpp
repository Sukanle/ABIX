...
TEST_CASE("7.version_evolution", "[version][prompt7]") {
    ...
    auto v1 = skl::abix::dll_func<void(const char *)>(lib, "log", SKL_ABIX_VERSION("1.0"));
    REQUIRE(v1.valid());
    v1("hello v1");
    auto lastmsg = skl::abix::dll_func<const char *()>(lib, "get_last_msg");
    auto level1 = skl::abix::dll_func<int()>(lib, "get_last_level");
    ...
    auto v2 = skl::abix::dll_func<void(const char *, int)>(lib, "log", SKL_ABIX_VERSION("2.0"));
    REQUIRE(v2.valid());
    v2("hello v2", 7);
    REQUIRE(level1() == 7);
...
TEST_CASE("18.version_evolution_guard", "[closed][prompt18]") {
    ...
    skl::abix::dll_object lib_v1;
    REQUIRE(lib_v1.load(dll_path("closed_dll").c_str()));
    auto type_hash_v1 = skl::abix::dll_func<uint64_t()>(lib_v1, "player_type_hash");
    ...
    skl::abix::dll_object lib_v2;
    if (!lib_v2.load(dll_path("closed_dll_v2").c_str())) {
        WARN("missing closed_dll_v2, skip version evolution test.");
        return;
    }

    auto type_hash_v2 = skl::abix::dll_func<uint64_t()>(lib_v2, "player_type_hash");
    auto offset_health_v2 = skl::abix::dll_func<int()>(lib_v2, "player_offset_health");
    ...
    auto create_v2 = skl::abix::dll_func<Player *(uint64_t)>(lib_v2, "create_player");
    auto destroy_v2 = skl::abix::dll_func<void(Player *)>(lib_v2, "destroy_player");
    auto get_health_v2 = skl::abix::dll_func<int(Player *)>(lib_v2, "player_get_health");
    auto alive_v2 = skl::abix::dll_func<int()>(lib_v2, "player_alive");

    REQUIRE(create_v2.valid());
    REQUIRE(alive_v2() == 0);

    {
        skl::abix::unique_dll_ptr<Player> p(create_v2(1), destroy_v2.raw());
        REQUIRE((bool)p);
