...
比较两个 ABIX artifact 时，工具加载两者，逐一查询类型和布局，生成逐类型的兼容性报告：

```mermaid
sequenceDiagram
    participant User as 用户
    participant AMC
    participant OldABIX as 旧版 ABIX
    participant NewABIX as 新版 ABIX
    participant Diff

    User->>AMC: compare(old, new)
    AMC->>OldABIX: load
    AMC->>NewABIX: load
    AMC->>Diff: compare ABI
    Diff->>OldABIX: query types/layouts
    Diff->>NewABIX: query types/layouts
    Diff-->>AMC: compatibility result
    AMC-->>User: ABI diff report
```

diff 结果将每个类型分类为下面的兼容性等级。
