...
```mermaid
graph LR
    subgraph Data["Data (required for runtime contract verification)"]
        A[type_id / layout_hash]
    end
    subgraph Symbols["Symbols (only needed for diagnostics)"]
        B[name / field name strings]
    end
```
...
The metadata travels through three stages from compilation to runtime use:
...
At compile time, AMC extracts ABI facts from the language AST and emits them
into binary sections (`.abix.metadata`, `.abix.names`). An offline tool can
scan or mmap the binary to read the Metadata Region directly, or call back into
AMC for decoded views. At runtime, the same Region is materialized into
pointer-rich descriptors for hot-path lookups.
...
```mermaid
graph TD
    A[release binary] -->|56 B descriptor, no .abix.names| D[production]
    B[relwithdebinfo binary] -->|56 B descriptor, no .abix.names| E[staging / crash analysis]
    B --> F[companion .abix artifact<br>contains all type/field names]
    C[debug binary] -->|72 B descriptor, .abix.names section| G[local development]
```
...
```mermaid
graph LR
    A[Crash stack] -->|extract type_id Hash128| B[archived .abix file]
    B --> C[retrieve name / layout / field information]
```
...
```mermaid
graph LR
    A[Production crash] -->|extract type_id hashes| B[ABIX symbol server]
    B -->|fetch .abix fragment| C[restore type names / field names / layout]
```
...
| Mode | Breakdown | Per-type |
|------|-----------|----------|
| Debug | 72 B TypeDescriptor (with name) + variable-length string constants | ≈ 120 B |
| Release | 56 B TypeDescriptor (without name) + no string constants | ≈ 56 B |
