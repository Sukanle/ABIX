...
```text
               Clang AST
                  ...
                  AMC
                   │
                   ▼
              AbiModule  ←── the only source of ABI truth
                   │
           ┌───────┴────────┐
           ▼                ▼
...
Clang AST → AMC → AbiModule
      ...
      (runtime projection)     amc_canonical_layouts[]
                                   (the only ABI truth)
                                     │
                                     ▼
                              RuntimeRegistry
                              (imports canonical data directly)
```
...
SOM:
  ...
  SOM Runtime
      ...
      ▼
  SOM ABI
  (the application must enter the SOM world)
...
ABIX:
           ...
           .abix      Runtime    AMC
              ...
              ▼         │          ▼
             CI       Binding    Package
  (ABIX does not take over the object model; it stands beside the native binary)
```
